...
- Luxart 14. 9. potvrdil stejnou REST instanci na interním portu 9759, Zone4You testovací databázi, HTTPS, IP allowlist a Basic Auth. IT musí dodat přesný veřejný HTTPS origin s certifikátem a reverse proxy na `9759`. Luxart zároveň potvrdil pětidenní logování klientského password hashe; protože je znovupoužitelný pro login, D1 zůstává zablokovaná do redakce všech citlivých login parametrů ve všech logovacích vrstvách;
...
Aktuální výsledek 14. 9. je `NO-GO` s 6/24 relevantními kontrolami pro zamýšlený `booking_without_payments`; čtyři Stripe kontroly jsou v této fázi mimo rozsah. Autoritou je vždy čerstvý výstup příkazu a přesný HEAD commit/Preview uvedený v draft PR, ne historické mezipočty níže.
...
| D2 | **Částečně potvrzeno 14. 9.:** Luxart potvrdil stejný REST kontrakt na interním `9759`, Zone4You testovací DB, HTTPS, IP allowlist a Basic Auth. Chybí veřejný hostname s certifikátem a proxy od IT a redakce znovupoužitelného password hashe ve všech logovacích vrstvách. | Zone4You IT + Luxart | živý read-only test a celou live integraci |
