...
- Luxart email dodal testovací klienty a IT potvrdilo dohodu o zveřejnění portu 9191. Veřejný kandidát `http://api.memberzone.online:9191/Service1.svc` je aktivní a obsahuje rezervační SOAP/WCF operace, ale není potvrzené, že jde o Zone4You test DB. Luxart s IT musí určit, zda je autoritativní tento SOAP kontrakt, nebo dříve dodaný REST kontrakt na portu 9295, a dodat šifrovaný přístup i auth režim;
...
| D2 | **Částečně potvrzeno 11. 9.:** port `9191` je veřejný, ale obsluhuje SOAP/WCF `Service1.svc`; REST dokumentace na `9295` je dostupná a datové endpointy bez credentials vracejí 401. Chybí společné rozhodnutí autoritativního kontraktu, potvrzení Zone4You test DB, přesný HTTPS origin a gateway auth režim. | Zone4You IT + Luxart | živý read-only test a celou live integraci |
...
1. Od IT Zone4You a Luxartu získat rozhodnutí `memberzone_rest_v1` versus SOAP/WCF, přesný HTTPS origin vybraného Zone4You test API a potvrzení test DB i auth režimu; termín v D1.
