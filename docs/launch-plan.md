...
- owner-only DNS capture uloží před změnou přesné A/AAAA/CNAME hodnoty bez jejich vypsání do konzole a bez TTL falešného driftu; těsně před změnou jediná předcutover brána v jednom běhu ověří celý schválený dossier i nezměněný DNS fingerprint a výsledek nepřepisovatelně uloží; všechny kritické owner-only JSONy se po zápisu i při čtení kontrolují bez symlinků, proti změně identity/velikosti/času a u nových důkazů byte-for-byte; pre-cutover receipt přenese přesný schválený počet lekcí, SHA-256 množiny výskytů, počet Reformerů, sedmidenní pražský rozsah, časové meze a dny; post-cutover brána přijme jen přesný hash tohoto čerstvého důkazu a stejný dossier, commit i fázi, poté ověří skutečnou produkční doménu, DNS/HTTPS, živý Luxart a přesnou shodu CS/EN feedu se schváleným snapshotem před otevřením pilotní skupiny; vynechaná/přidaná lekce nebo nový pražský den je NO-GO a při chybě se vrací uložené DNS hodnoty;
