...
**Aktuální autoritativní regrese 13. 9.:** po zavedení D1 atestace verze 6, povinné kontroly veřejného kořene, least-privilege pořadí anonymní sonda → gateway → kontrakt → testovací klient a oddělených hashovaných live login limitů na zdroj i účet prošlo 240/240 jednotkových a integračních testů, produkční sestavení, 62/62 lokálních Playwright scénářů, audit závislostí bez známých zranitelností a úplný scan aktuálních souborů i Git historie bez nálezu secretu; 13 nevhodných browser kombinací zůstalo záměrně přeskočeno. Testy i launch brána navíc vážou skutečnou login routu na přesné pořadí připravenost → limit zdroje → načtení a validace vstupu → limit účtu → Luxart → vytvoření session; odmítnutí na dřívější bráně nepokračuje k citlivějším krokům. Tento odstavec nahrazuje všechny starší mezipočty výše.
