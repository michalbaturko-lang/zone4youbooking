...
- streamovaný limit 4 MiB na JSON odpovědi Luxartu; nadlimitní nebo neplatná odpověď se nevypíše a u zápisové operace vyvolá ruční reconciliation bez automatického opakování;
...
**Aktualizace plné regrese 11. 9.:** obnovená bezpečná větev prošla 124/124 integračními a jednotkovými testy, produkčním sestavením a 49/49 provedenými lokálními Playwright scénáři na 320 × 568, 390 × 844, 768 × 1024, landscape 844 × 390 a 1440 × 900; jedna desktopová duplicita čistě mobilního ergonomického testu je záměrně přeskočena. Izolovaný veřejný Vercel Preview navíc prošel 20/20 nízkoobjemovými browser scénáři ve stejné matici, včetně jediného loginu, rezervace a storna; 5 nadbytečných/nevhodných variant bylo záměrně přeskočeno, aby test neobcházel login rate limit. Dependency audit hlásí 0 známých zranitelností. Zone4You IT potvrdilo dohodu o zveřejnění portu 9191, nikoli však přesný host, HTTPS, aktivní dostupnost, testovací databázi ani gateway auth; live integrace a cutover proto zůstávají `NO-GO`.
