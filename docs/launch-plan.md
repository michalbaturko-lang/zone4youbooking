...
- finální release-dossier gate svazuje přesný commit a launch okno s hashovanými živými důkazy Luxartu, úplností feedu/sálů, UAT, alertu, pětiminutového aplikačního rollbacku, čerstvého přesného DNS rollback baseline, čerstvého owner-only technického i ručního potvrzení Memberzone fallbacku a výslovným cutover souhlasem; fallback kontrola ukládá jen metadata a SHA-256, nikdy HTML ani osobní údaje;
- bezpečný dossier preparátor automatizuje pouze kontrolu souborů a SHA-256; výstup je vždy nepřepisovatelný NO-GO draft a finální brána odmítne placeholder schvalující osoby, nepotvrzený alert, chybějící jmenovité potvrzení aktivních Luxart e-mailových šablon i schválení časově starší než příslušný důkaz; cutover musí být posledním schválením a jeho čas musí předcházet pre-cutover receipt i následné produkční kontrole;
- statická launch brána vyžaduje přesné `package.json` zapojení celého operátorského řetězce: D1, runtime, mutační UAT, start a ověření rollbacku, zachycení i kontrolu DNS baseline, alert, Memberzone fallback, přípravu i validaci dossieru a pre/post-cutover kontrolu; pouhá existence části souborů proto nevytvoří falešně kompletní provozní cestu;
- owner-only DNS capture uloží před změnou přesné A/AAAA/CNAME hodnoty bez jejich vypsání do konzole a bez TTL falešného driftu; těsně před změnou jediná předcutover brána v jednom běhu ověří celý schválený dossier i nezměněný DNS fingerprint a výsledek nepřepisovatelně uloží; všechny kritické owner-only JSONy se po zápisu i při čtení kontrolují bez symlinků, proti změně identity/velikosti/času a u nových důkazů byte-for-byte; pre-cutover receipt přenese přesný schválený počet lekcí, SHA-256 množiny výskytů, SHA-256 vazeb výskyt → číslo sálu, množinu čísel sálů, počet Reformerů, sedmidenní pražský rozsah, časové meze a dny; post-cutover brána přijme jen přesný hash tohoto čerstvého důkazu a stejný dossier, commit i fázi, poté ověří skutečnou produkční doménu, DNS/HTTPS, živý Luxart, přesnou shodu CS/EN feedu se schváleným snapshotem a v témže okamžiku také anonymní dostupnost původního Memberzone scheduleru s neprázdným rozvrhem a Reformerem; zelený výsledek sama uloží jako nový nepřepisovatelný owner-only soubor mimo repozitář a znovu ověří jeho SHA-256, takže pouhý terminálový výpis nemůže autorizovat otevření pilotu; vynechaná/přidaná lekce, změněný sál, nový pražský den nebo nedostupný fallback je NO-GO a při chybě se vrací uložené DNS hodnoty;
- fail-closed deployment preflight odděluje read-only, booking bez plateb a booking se Stripe, odmítá CLI/test secrets v runtime prostředí a váže readiness i release dossier na přesný commit a fázi; post-cutover kontrola navíc přijme pouze hash čerstvého nepřepisovatelného pre-cutover důkazu se stejným dossierem, commitem a fází;
- booking readiness, release dossier, pre-cutover receipt a post-cutover kontrola navíc nesou stejný kanonický SHA-256 celé mapy `cislo_salu → id_resource`; odlišná stagingová nebo produkční mapa je NO-GO bez zveřejnění interních resource ID;
...
- anglická lokalizace je implementovaná lokálně; živé anglické Luxart texty je nutné ověřit proti potvrzené Zone4You REST instanci;
...
- Luxart email dodal testovací klienty a potvrzuje, že objednaná REST instance běží na serveru Zone4You na interním portu 9759 proti testovací databázi. IT potvrdilo zveřejnění portu 9191, ale nedodalo veřejný hostname/IP ani potvrzení překladu `9191 → 9759`. SOAP/WCF na `api.memberzone.online:9191` je odlišný kontrakt a není důkazem klientské instance. IT musí dodat přesný šifrovaný REST origin a auth režim;
...
Aktuální výsledek 13. 9. je `NO-GO` s 6/24 relevantními kontrolami pro zamýšlený `booking_without_payments`; čtyři Stripe kontroly jsou v této fázi mimo rozsah. Autoritou je vždy čerstvý výstup příkazu a přesný HEAD commit/Preview uvedený v draft PR, ne historické mezipočty níže.
**Aktualizace plné regrese 13. 9.:** obnovená bezpečná větev prošla 233/233 integračními a jednotkovými testy, produkčním sestavením a 62/62 provedenými lokálními Playwright scénáři v Chromium na 320 × 568, 390 × 844, 768 × 1024, landscape 844 × 390 a 1440 × 900 plus ve WebKitu na 390 × 844; 13 kombinací projektu a scénáře je záměrně přeskočeno. Izolovaný veřejný Vercel Preview navíc prošel 24/24 nízkoobjemovými browser scénáři v pěti Chromium viewpor-tech a read-only WebKitu, včetně jediného demo loginu a vytvoření/storna běžné lekce i Reformeru v nejmenším Chromium viewportu; 5 nadbytečných zapisujících variant je záměrně přeskočeno, aby test neobcházel login rate limit. Živé mutační UAT používá pro všechny mapované sály jedno ověřené přihlášení a před každým sálem znovu kontroluje přesný deployment. Lokální WebKit prochází i přihlášením, rezervací a stornem běžné lekce a Reformeru proti izolovanému mocku; živý iOS/Safari test zůstává stagingovým UAT. Demo je v obou jazycích viditelně označené jako nepropojené s Memberzone a kompaktní mobilní varianta zachovává první lekci nad spodní navigací i na 320 × 568. Externí běh je před prvním loginem vázaný na přesný 40znakový Git commit a systémový region `fra1`. Watchdog mutace mají samostatný fail-closed přepínač a omezený pilot je musí držet vypnuté; budoucí smazání navíc vyžaduje, aby se záznam znovu našel ve vlastním Luxart seznamu přihlášeného klienta. Mapy názvů sálů/typů i `cislo_salu` → `id_resource` používají společnou striktní validaci v preflightu, runtime a release dossieru; žádná chybná položka se tiše nezahodí ani nedostane do UI. Luxart ID se přijímají jen jako kanonická bezpečná celá čísla a payload rezervace/watchdogu musí přesně souhlasit s kategorií, službou a časem zakódovanými v ID výskytu. Adaptér odmítá duplicitní ID výskytu už při interním znovunačtení rozvrhu před rezervací, takže konfliktní Luxart řádky nemohou tiše vybrat chybný sál ani `id_resource` a nevznikne žádný zápis. Neznámý Luxart `success` kód, včetně `0` nebo jiné kladné hodnoty mimo povolené `1`/`2`, nemůže vyrobit falešné potvrzení storna rezervace ani watchdogu. Idempotentní ledger odmítne jako nejistý každý Luxart výsledek s jiným klientem nebo výskytem, nekonzistentní kategorií, časem bez zóny či zápornou cenou nebo poplatkem; bezpečné storno musí obsahovat i přesný identifikátor, zónovaný čas a poplatek včetně nuly. Veřejné JSON/XML příklady Luxartu dokládají čas s `Z` nebo číselným offsetem; adapter u lekcí, rezervací, kreditu i watchdogu odmítá čas bez zóny, neexistující datum a nepřípustný offset, aby výsledek nezávisel na časové zóně hostingu. Zobrazení i rezervační předkontrola používají přímo Luxart `volno`; členské kvóty nemůže přepsat prostý dopočet `kapacita - obsazeno` a nekonzistentní dostupnost zneplatní feed. D1 brána nezakazuje potenciálně legitimní hostname, ale vyžaduje výslovně schválený SHA-256 otisk přesného HTTPS originu a shodu transportního i autentizovaného důkazu. Její soubor obsahuje samostatnou D1 atestaci a dossier schématu 6 ji povinně validuje společně s čerstvým owner-only Memberzone fallback důkazem a nepřepisovatelnou owner-only startovní rollback účtenkou svázanou SHA-256 s výsledkem drillu, takže samostatný diagnostický read-only výstup nemůže obejít `/Help` a origin gate ani zpětně dopsat začátek pětiminutového měření. Pre-cutover receipt přenese přesný schválený snapshot lekcí a produkční brána odmítne jediný vynechaný či přidaný výskyt, změnu počtu Reformerů i nový pražský den bez nových release důkazů. Kritické release JSON vstupy se otevírají stabilně bez následování symlinků a s detekcí změny identity, velikosti, práv nebo obsahu; DNS baseline, Memberzone fallback, rollback timer i pre-cutover receipt zůstávají owner-only. Dependency audit hlásí 0 známých zranitelností. Přímé ověření 13. 9. v 20:41 CEST znovu potvrdilo, že `api.memberzone.online:9191/Service1.svc` je aktivní rezervační SOAP/WCF služba: 4/4 dokumenty, 8/8 požadovaných operací a 8/8 odpovědních tabulek prošly, ale `/Help` vrací 404, directory listing zůstává zapnutý a HTTPS není funkční. Veřejný SOAP na 9191 není potvrzenou Zone4You instancí; IT musí dodat veřejný host a potvrdit překlad na interní REST port 9759. Bez potvrzení test DB a šifrovaného přístupu zůstávají live integrace a cutover `NO-GO`.
**Novější doplněk regrese 13. 9.:** po přidání dynamického mobile-first feedu, před-UAT validace resource mapy a lokálního autentizovaného WebKit/Safari průchodu prošlo 211/211 jednotkových a integračních testů a 62/62 provedených Playwright scénářů; 13 nevhodných kombinací projektu a scénáře bylo záměrně přeskočeno. Referenční mobil 390 × 844 prokázal úplné zobrazení 37 položek a automatické filtry pro dosud neznámé sály bez horizontálního overflow či ovládání pod 44 px. WebKit ve stejném viewportu proti izolovanému mocku navíc prošel přihlášením, vytvořením a stornem běžné lekce i Reformeru; veřejný WebKit Preview zůstává bez loginu a živé Luxart Safari UAT je nadále povinné. Tento výsledek nahrazuje počty v předchozím odstavci.
...
**Bezpečnostní doplněk 13. 9.:** login audit potvrdil odmítání redirectu a bezpečné aplikační chyby/logy, ale odhalil infrastrukturní riziko samotného Luxart REST kontraktu: login, MD5 hesla a volitelná karta jsou v query stringu `/api/Login`. D1 atestace verze 6, preflight, resource-map mezibrána i release dossier proto odmítnou live login bez písemného potvrzení, že query string neukládá ani reverzní proxy, ani webserver, ani monitoring, případně že všechny tři citlivé parametry redigují. D1 verze 6 před loginem navíc bezpečně ověří veřejný kořen stejného originu a odmítne otevřený nebo neověřitelný directory listing. Potvrzení logů váže na skutečného schvalujícího člověka/roli a čas nejvýše 30 dní před testem; placeholder, budoucí nebo starší potvrzení odmítne. Celá lokální regrese prošla 223/223 testy, produkčním sestavením a 62/62 provedenými Playwright scénáři; 13 kombinací zůstalo záměrně přeskočeno. Tento odstavec nahrazuje předchozí lokální počty.

**Aktuální autoritativní regrese 13. 9.:** po zavedení D1 atestace verze 6, povinné kontroly veřejného kořene, least-privilege pořadí anonymní sonda → gateway → kontrakt → testovací klient a oddělených hashovaných live login limitů na zdroj i účet prošlo 250/250 jednotkových a integračních testů, produkční sestavení, 62/62 lokálních Playwright scénářů, audit závislostí bez známých zranitelností a úplný scan aktuálních souborů i Git historie bez nálezu secretu; 13 nevhodných browser kombinací zůstalo záměrně přeskočeno. Testy i launch brána vážou skutečnou login routu na přesné pořadí připravenost → limit zdroje → načtení a validace vstupu → limit účtu → Luxart → vytvoření session. Stejně vážou rezervaci i storno na pořadí důvěryhodný Origin → booking brzda → podepsaná session → live readiness → tělo → idempotency → limit → Luxart adapter → durable ledger → mutace; odmítnutí na dřívější bráně nepokračuje k citlivějším krokům. Nový anonymní intake před D1 přijímá jen přesný HTTPS cíl a bez načtení jakýchkoli credentials váže `/Help`, bezpečný veřejný kořen a 11/11 částí REST kontraktu na jediný otisk; jeho výsledek sám neautorizuje login ani launch. Tento odstavec nahrazuje všechny starší mezipočty výše.
...
Řízené mutační UAT povinně provede jeden oddělený a plně obnovený scénář nad různým výskytem pro každý sál v úplné Luxart resource mapě; sada musí zahrnout běžnou lekci i Reformer. Každý scénář se zastaví ještě před prvním zápisem, pokud neodpovídá požadovanému typu a přesnému sálu, nemá explicitní personalizované povolení, autoritativní volné místo, otevřené rezervační okno nebo bezpečně dostupné online storno se shodným očekávaným poplatkem. Finální dossier odmítne vynechaný či duplicitní sál, chybějící typ, opakovanou lekci nebo rezervaci i opakované korelační ID; tyto předpodmínky proto nejsou jen operátorským checklistem.
...
| D2 | **Částečně potvrzeno 11. 9.:** objednaný kontrakt je REST a interní Zone4You instance má běžet na portu `9759` proti testovací DB; referenční dokumentace na `9295` je dostupná a datové endpointy bez credentials vracejí 401. IT potvrdilo zveřejnění `9191`, ale chybí veřejný hostname/IP, potvrzení překladu, přesný HTTPS origin a gateway auth režim. | Zone4You IT + Luxart | živý read-only test a celou live integraci |
...
Sedmidenní termín je dosažitelný pouze jako přísně řízený pilot všech `api/Lesson` položek. Potvrzený REST kontrakt, přesný HTTPS origin a bezpečný testovací účet musí být funkční v D1; každý další den prodlení přímo ubírá den z integračního testování.
