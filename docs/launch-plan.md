...
**Aktuální autoritativní regrese 13. 9.:** po zavedení D1 atestace verze 6, povinné kontroly veřejného kořene, least-privilege pořadí anonymní sonda → gateway → kontrakt → testovací klient a oddělených hashovaných live login limitů na zdroj i účet prošlo 246/246 jednotkových a integračních testů, produkční sestavení, 62/62 lokálních Playwright scénářů, audit závislostí bez známých zranitelností a úplný scan aktuálních souborů i Git historie bez nálezu secretu; 13 nevhodných browser kombinací zůstalo záměrně přeskočeno. Testy i launch brána navíc vážou skutečnou login routu na přesné pořadí připravenost → limit zdroje → načtení a validace vstupu → limit účtu → Luxart → vytvoření session; odmítnutí na dřívější bráně nepokračuje k citlivějším krokům. Nový anonymní intake před D1 přijímá jen přesný HTTPS cíl a bez načtení jakýchkoli credentials váže `/Help`, bezpečný veřejný kořen a 11/11 částí REST kontraktu na jediný otisk; jeho výsledek sám neautorizuje login ani launch. Tento odstavec nahrazuje všechny starší mezipočty výše.
