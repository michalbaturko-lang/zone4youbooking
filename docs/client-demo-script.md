...
- Autoritativní regrese 13. 9.: aktuální větev prošla 250/250 jednotkovými a integračními testy, produkčním sestavením a 62/62 lokálními browser scénáři; poslední dokončený veřejný Preview běh prošel 24/24 nízkoobjemovými scénáři. Přesný HEAD commit a URL se berou z draft PR, ne z této statické prezentace. Chromium i WebKit pokrývají mobile-first rozvrh, CS/EN, přihlášení, rezervaci a storno běžné lekce i Reformeru proti izolovanému mocku; 13 lokálních a 5 veřejných nevhodných kombinací je záměrně přeskočeno. Runtime potvrzuje 24 lekcí, všechny tři sály, 3 Reformery, sedm pražských dnů a region `fra1`. Bezpečnostní audit závislostí i celé Git historie je čistý. Živé mutace navíc vyžadují serverovou session před načtením těla, limiterem, Luxart adaptérem a ledgerem. Živé UAT musí otestovat každý mapovaný sál a používá jeden ověřený login pro celou sadu; demo tento důkaz nenahrazuje.
