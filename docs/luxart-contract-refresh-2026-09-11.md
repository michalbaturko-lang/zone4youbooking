...
Adaptér lze dál vyvíjet a lokálně testovat proti REST kontraktu na `9295`. Současný `RealLuxartAdapter` volá JSON endpointy `/api/Login`, `/api/User`, `/api/Lesson`, `/api/Reservations`, `/api/Watchdog` a `/api/Payment`; se SOAP/WCF `Service1.svc` na `9191` není kompatibilní a ani jej nepotřebuje. Živý D1 se otevře až po potvrzení přesného veřejného Zone4You cíle, překladu na interní REST port `9759`, testovací databáze, bezpečného šifrovaného přístupu a redakce login query stringu v přístupových logách. D1 evidence verze 6 musí před klientským loginem doložit, že stejný schválený HTTPS origin zveřejňuje všech 11 dokumentačních stránek se stejným sémantickým SHA-256 jako referenční REST kontrakt, nemá veřejný directory listing a že ochranu logů IT/Luxart výslovně potvrdili konkrétní osobou/rolí a časem nejvýše 30 dní před testem; tuto vazbu následně znovu ověřuje finální release dossier. Rezervační zápisy zůstanou vypnuté až do samostatně povoleného UAT, mapování všech pozorovaných sálů, PostgreSQL ledgeru, úplných business pravidel, rollbacku a výslovného cutover schválení.

SOAP by se stal integrační variantou pouze při výslovné změně rozsahu potvrzené Zone4You i Luxartem. V takovém případě by před implementací musely být doplněny významy `GetReservation`, `ID_ACTIVITY_IN`, identifikátorů, osobní rezervovatelnosti, jazyků, watchdogu a chybových kódů. V současném projektu je SOAP audit pouze negativní srovnávací důkaz a nesmí měnit `LUXART_API_CONTRACT` ani otevřít release.
