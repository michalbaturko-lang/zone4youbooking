...
| Klientský REST drift | D1 na schváleném Zone4You HTTPS originu před loginem ověří stejných 11 dokumentačních stránek, stejný sémantický baseline a vypnutý veřejný directory listing; odchylka nevytvoří evidence | D1 evidence verze 6 + finální dossier validace |
...
| Login query logy | proxy, webserver i monitoring Luxartu vynechají nebo redigují `login`, `password` a `member_card_number` v query stringu `/api/Login`; bez potvrzení se skutečný login neodešle | písemné potvrzení IT/Luxartu + `LUXART_LOGIN_QUERY_LOGGING_CONFIRMED=true` + skutečný schvalující/role a čas do 30 dní + D1 evidence verze 6 |
...
| Předmutací UAT | dva různé výskyty — běžná lekce a Reformer — jsou pro testovacího klienta výslovně povolené, mají autoritativní volné místo a mapovaný sál, jsou v potvrzeném rezervačním okně a lze je bezpečně stornovat s očekávaným poplatkem podle vlastního pravidla; jinak nevznikne platný suite důkaz | privacy-safe dvojitý UAT preflight důkaz + negativní testy typu a zápisu |
...
| Produkční doména po cutoveru | před otevřením pilotní skupiny `verify:production-cutover` do 60 s prokáže DNS, platné HTTPS hlavičky, schválený commit/fázi, živý Luxart, PostgreSQL rate limit, připravený booking a CS/EN feed přesně shodný se schváleným počtem, SHA-256 množiny výskytů, samostatnými lokalizovanými SHA-256 statického obsahu, počtem Reformerů, sedmidenním pražským rozsahem, mezemi a dny | privacy-safe JSON svázaný s přesným owner-only pre-cutover souborem mladším 30 minut a stejným dossierem; vynechaná/přidaná lekce, změna času, názvu, instruktora, kapacity či ceny nebo nový pražský den vyžaduje nový důkaz; bez DNS adres, ID lekcí, secrets a osobních dat; chyba spouští DNS rollback |
...
- **Autoritativní stav 13. 9.:** aktuální větev prošla 250/250 jednotkovými a integračními testy, produkčním sestavením a 62/62 lokálními Playwright scénáři; přesný HEAD commit, veřejný Preview a stav CI/Vercelu se vedou v draft PR, aby tento dokument nevydával starší deploy za aktuální. Poslední dokončený veřejný běh prošel 24/24 nízkoobjemovými scénáři. Pět veřejných zapisujících variant je záměrně přeskočeno kvůli login limiteru, 13 lokálních kombinací nemá pro svůj projekt smysl. Mutační UAT nyní vyžaduje samostatně obnovený výskyt v každém namapovaném sále, včetně běžné lekce i Reformeru, a pro celou sadu jediný ověřený login. Živá rezervace i storno nyní ověří podepsanou session a live readiness před načtením těla, limitem, adaptérem a ledgerem; testy a launch brána vážou stejné pořadí pro obě routy. Post-cutover kontrola znovu anonymně ověří dostupnost starého Memberzone rozvrhu, neprázdné lekce a Reformer a uloží jen privacy-safe metadata a hash. Aktuální statická brána je `NO-GO`: 8/24 pro `booking_without_payments`; čtyři Stripe kontroly jsou v této fázi mimo rozsah. Autoritou pro další rozhodnutí je nový výstup testů a `npm run check:launch`, následující mezistavy jsou historický audit trail.
- **Aktuální plná regrese 13. 9.:** obnovená větev prošla 207/207 integračními a jednotkovými testy, produkčním buildem, 60/60 provedenými lokálními Playwright scénáři v šesti projektech a dvou enginech, 9 záměrně přeskočenými kombinacemi projektu a scénáře a auditem s 0 známými dependency zranitelnostmi. Předchozí izolovaný veřejný Preview přesného commitu `ca5c085ed7a820ecb78c678a1ab6c661a8d81f46` a regionu `fra1` prošel 20/20 nízkoobjemovými scénáři v původních pěti Chromium viewpor-tech včetně vytvoření/storna běžné lekce i Reformeru; 5 variant bylo záměrně přeskočeno kvůli login limiteru a desktopové mobilní ergonomii. Jednorázový mobilní Lighthouse lab nad týmž Preview naměřil výkon 97/100, přístupnost 100/100, LCP 2,344 s, TBT 2 ms a CLS 0; produkční p75 a INP zůstávají staging/UAT důkazem. Samostatný mobilní WebKit 390 × 844 bez loginu a mutací ověřuje úplnost rozvrhu, ergonomii, WCAG/focus flow, angličtinu a oblíbené. Externí brána před browser testem vyžaduje shodu přesného 40znakového Git commitu a regionu `fra1`. Watchdog zápisy jsou pro omezený pilot samostatně uzamčené; před smazáním se záznam musí znovu najít ve vlastním Luxart seznamu přihlášeného klienta. Konfigurace názvů sálů/typů a rezervačních zdrojů se validuje shodně v preflightu, runtime i release dossieru; neplatná položka se nesmí tiše zahodit ani rozbít UI až po načtení živého rozvrhu. ID klienta, lekce, služby, kategorie, sálu a zdroje musí být bezpečná celá čísla a rezervační payload musí přesně souhlasit s identitou výskytu lekce. Ledger přijme nebo znovu přehraje úspěch jen pro stejného klienta a výskyt lekce, s explicitně zónovanými časy, shodnou kategorií a nezápornou cenou; storno navíc vyžaduje svůj přesný identifikátor, čas a nezáporný Luxart poplatek včetně nuly. Luxart časy u lekcí, rezervací, kreditu i watchdogu musí mít explicitní `Z` nebo číselný offset a existující kalendářní datum; jiný vstup selže bezpečně bez převodu podle hostingu. Dostupnost míst používá autoritativní Luxart `volno` a nekonzistentní kapacitu odmítne, takže členskou kvótu nepřepíše odvozený počet. Dossier schématu 6 přijme pouze výstup složené D1 brány s atestací `/Help`, schváleného originu, gateway režimu a autentizovaného čtení, samostatný čerstvý Memberzone fallback důkaz a owner-only startovní rollback účtenku svázanou SHA-256 s výsledkem drillu; samostatný diagnostický read-only soubor nestačí. Pre-cutover receipt nově přenáší přesný schválený snapshot lekcí a produkční brána odmítne jediný vynechaný či přidaný výskyt, změnu Reformerů i přechod do nového pražského dne. Všechny kritické release JSON vstupy včetně DNS baseline, rollback timeru a pre-cutover receipt se nyní čtou bez následování symlinků, s limitem velikosti, kontrolou oprávnění a detekcí změny identity nebo obsahu během čtení. Tento výsledek nahrazuje archivní počty v následujícím detailu.
- **Novější doplněk regrese 13. 9.:** po přidání dynamického mobile-first feedu, před-UAT validace resource mapy a lokálního autentizovaného WebKit/Safari průchodu prošlo 211/211 jednotkových a integračních testů a 62/62 provedených Playwright scénářů; 13 nevhodných kombinací projektu a scénáře bylo záměrně přeskočeno. Test nad 37 položkami prokazuje, že UI nemá pevný limit 24 lekcí, zachová každý výskyt a automaticky vytvoří filtry pro dosud neznámé sály bez zmenšení ovládání pod 44 px nebo horizontálního overflow. WebKit 390 × 844 proti izolovanému mocku navíc ověřuje celý přihlášený tok pro běžnou lekci i Reformer včetně storna; veřejný Preview zůstává v tomto enginu read-only a živé Luxart Safari UAT tato lokální kontrola nenahrazuje. Tento bod nahrazuje počty v předchozím odstavci.
- **Nejnovější lokální regrese 13. 9.:** validace demo stavu, povinné oddělení běžného/Reformer UAT, přesná PostgreSQL schema atestace a runtime kontrola úplné resource mapy zvýšily sadu na 221/221 jednotkových a integračních testů; produkční sestavení a 62/62 provedených browser scénářů prošly, 13 nevhodných kombinací bylo záměrně přeskočeno. Chromium i WebKit simulují zablokované čtení, zápis a mazání úložiště a přesto dokončí přihlášení, rezervaci a storno; poškozený nebo nadlimitní stav se bezpečně zahodí a nikdy se neodešle na server. Mutační UAT nyní vyžaduje dva různé výskyty, ověří jejich správný typ a u každého před zápisem odmítá patologický kredit, cenu či storno pravidlo; finální dossier vyžaduje oba obnovené scénáře, omezené částky, explicitně zónované časy, výslovný storno poplatek včetně nuly a unikátní korelační ID napříč dvojicí. Readiness booking ledgeru a rate limiteru nyní odmítne tabulku se správným počtem, ale nesprávnou identitou primárního nebo unikátního klíče; rozšířené PostgreSQL integrační testy tuto záměnu simulují v izolovaném schématu. Rate limiter při každém zápisu opportunisticky odstraňuje jen omezenou dávku bucketů starších než ochranná lhůta; PostgreSQL test dokazuje maximum 100 smazání, zachování aktivního okna a obnovu právě používaného klíče. Booking readiness navíc porovná aktuální živá čísla sálů s celou nasazenou mapou a při chybějícím, přidaném nebo nekladném čísle vrátí 503 s `resource_map_mismatch` dříve, než pilot povolí rezervace. Tento bod nahrazuje předchozí lokální počty; externí důkaz následuje po nasazení přesného commitu.
- **Bezpečnostní doplněk 13. 9.:** audit celé login cesty potvrdil, že adaptér odmítá redirect a aplikační chyby ani logy nevracejí login, heslo, jeho MD5 ani členskou kartu. Protože Luxart REST kontrakt přesto přenáší tyto hodnoty v query stringu `/api/Login`, D1 atestace verze 6, deployment preflight, resource-map mezibrána i finální release dossier fail-closed vyžadují písemné potvrzení, že proxy, webserver a monitoring query string neukládají nebo citlivé parametry redigují. D1 verze 6 navíc před klientským loginem ověřuje vypnutý veřejný directory listing a váže potvrzení logů na skutečného schvalujícího člověka nebo provozní roli a čas nejvýše 30 dní před testem; placeholder, budoucí nebo starší potvrzení odmítne. Celá regrese po změně prošla 223/223 jednotkovými a integračními testy, produkčním sestavením a 62/62 provedenými browser scénáři; 13 nevhodných kombinací bylo záměrně přeskočeno. Tento bod nahrazuje předchozí lokální počty.
- Cena lekce, kredit klienta, historie kreditu, storno poplatek, demo stav a idempotentní booking ledger sdílejí technický limit 1 miliarda Kč v absolutní hodnotě. Vyšší upstream číslo je neplatné nebo nejisté, nikoli finanční úspěch; limit nenahrazuje obchodní pravidla Zone4You.
