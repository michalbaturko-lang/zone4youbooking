...
- **Autoritativní stav 13. 9.:** commit `20895127ec42dcb57fa35374791c7f5d47081108` prošel 226/226 jednotkovými a integračními testy, produkčním sestavením, 62/62 lokálními Playwright scénáři a 24/24 nízkoobjemovými scénáři nad přesným veřejným Preview; CI a Vercel jsou zelené. Pět veřejných zapisujících variant je záměrně přeskočeno kvůli login limiteru, 13 lokálních kombinací nemá pro svůj projekt smysl. Post-cutover kontrola nyní znovu anonymně ověří i dostupnost starého Memberzone rozvrhu, neprázdné lekce a Reformer a uloží jen privacy-safe metadata a hash. Aktuální statická brána je `NO-GO`: 6/28 bez fáze, 7/24 pro `booking_without_payments`; čtyři Stripe kontroly jsou v této fázi mimo rozsah. Autoritou pro další rozhodnutí je nový výstup testů a `npm run check:launch`, následující mezistavy jsou historický audit trail.
