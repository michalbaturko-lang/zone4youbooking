...
- **Autoritativní stav 13. 9.:** aktuální větev prošla 240/240 jednotkovými a integračními testy, produkčním sestavením a 62/62 lokálními Playwright scénáři; přesný HEAD commit, veřejný Preview a stav CI/Vercelu se vedou v draft PR, aby tento dokument nevydával starší deploy za aktuální. Poslední dokončený veřejný běh prošel 24/24 nízkoobjemovými scénáři. Pět veřejných zapisujících variant je záměrně přeskočeno kvůli login limiteru, 13 lokálních kombinací nemá pro svůj projekt smysl. Mutační UAT nyní vyžaduje samostatně obnovený výskyt v každém namapovaném sále, včetně běžné lekce i Reformeru, a pro celou sadu jediný ověřený login. Post-cutover kontrola znovu anonymně ověří dostupnost starého Memberzone rozvrhu, neprázdné lekce a Reformer a uloží jen privacy-safe metadata a hash. Aktuální statická brána je `NO-GO`: 6/24 pro `booking_without_payments`; čtyři Stripe kontroly jsou v této fázi mimo rozsah. Autoritou pro další rozhodnutí je nový výstup testů a `npm run check:launch`, následující mezistavy jsou historický audit trail.
