...
| Klientský REST drift | D1 na schváleném Zone4You HTTPS originu před loginem ověří stejných 11 dokumentačních stránek a stejný sémantický baseline; odchylka nevytvoří evidence | D1 evidence verze 4 + finální dossier validace |
...
| Login query logy | proxy, webserver i monitoring Luxartu vynechají nebo redigují `login`, `password` a `member_card_number` v query stringu `/api/Login`; bez potvrzení se skutečný login neodešle | písemné potvrzení IT/Luxartu + `LUXART_LOGIN_QUERY_LOGGING_CONFIRMED=true` + D1 evidence verze 4 |
...
- **Bezpečnostní doplněk 13. 9.:** audit celé login cesty potvrdil, že adaptér odmítá redirect a aplikační chyby ani logy nevracejí login, heslo, jeho MD5 ani členskou kartu. Protože Luxart REST kontrakt přesto přenáší tyto hodnoty v query stringu `/api/Login`, D1 atestace verze 4, deployment preflight, resource-map mezibrána i finální release dossier nově fail-closed vyžadují písemné potvrzení, že proxy, webserver a monitoring query string neukládají nebo citlivé parametry redigují. Celá regrese po změně prošla 223/223 jednotkovými a integračními testy, produkčním sestavením a 62/62 provedenými browser scénáři; 13 nevhodných kombinací bylo záměrně přeskočeno. Tento bod nahrazuje předchozí lokální počty.
