...
| Rollback | návrat do read-only je do 5 minut od UTC času zachyceného před změnou konfigurace automaticky ověřen `verify:readonly-rollback`; ověřený runtime má stejný commit, fázi `read_only` a region `fra1`; přesná HTTPS fallback URL Memberzone vrací neprázdný Zone4You rozvrh včetně Reformeru a dostupnost současně ručně potvrdí odpovědná osoba | privacy-safe owner-only JSON z `verify:memberzone-fallback`, celý SHA-256 a časovaný rollback záznam se začátkem a okamžikem dosažení read-only; bez uloženého HTML nebo osobních údajů |
