...
- [ ] Guarded `verify:booking-mutations` proběhl pouze na schválené test DB, odmítl produkční origin a před jediným sdíleným loginem svázal `/api/readiness` s přesným commitem, launch fází, regionem `fra1`, capability profilem a `resourceMapSha256` úplné operátorské mapy. Provedl jeden oddělený scénář nad různým výskytem pro každý mapovaný sál, včetně běžné lekce i Reformeru, a před každým prvním POSTem bez dalšího loginu znovu ověřil readiness, správný typ, přesné číslo sálu, personalizované povolení, autoritativní `volno`, rezervační okno, možnost online storna a očekávaný poplatek; uložil jeden privacy-safe JSON důkaz autentizace a všech obnovených stavů.
