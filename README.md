...
`verify:booking-mutations` smí běžet jen nad výslovně potvrzenou testovací databází. Ještě před prvním zápisem vyžaduje přesnou shodu staging commitu a launch fáze z `/api/readiness`, region `fra1`, live Luxart a vypnutý waitlist; u pilotu bez plateb také vypnuté top-upy. `ZONE4YOU_UAT_ROOM_SCENARIOS_JSON` musí obsahovat právě jeden různý výskyt pro každý sál v úplném `LUXART_RESOURCE_MAP_JSON` a soubor musí zahrnout běžnou lekci i Reformer. U každého sálu skript vyžaduje explicitní personalizované povolení, autoritativní volné místo, přesnou shodu čísla sálu, potvrzené rezervační okno a bezpečné online storno s očekávaným poplatkem podle pravidla daného typu. Před zápisem i u každé odpovědi rezervace/storna používá stejné limity částek a stejnou kontrolu explicitně zónovaných časů jako živá aplikace; chybějící storno poplatek není tiše zaměněn za nulu. Výsledný UAT důkaz má povinnou verzi schématu a finální release dossier znovu nezávisle ověří úplné pokrytí mapy, oba typy lekcí, rozdílné výskyty a rezervace, částky, časy a korelační ID. Odpověď na storno sama nestačí: až následný Luxart snapshot smí potvrdit, že každá UAT rezervace zmizela. Současně se u všech scénářů porovná přesná množina identit všech dříve aktivních rezervací klienta. Jakýkoli nesoulad končí bez rezervačního POSTu nebo neúspěšným UAT důkazem.
