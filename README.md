...
`probe:luxart-help` je první bezpečný síťový test po získání přesné URL. Posílá neautentizovaný `GET /Help` a u použitelné dokumentace nebo gateway výzvy také jediný `GET /` stejného originu, zakazuje redirecty, omezuje čas i velikost odpovědi a do výsledku ukládá jen port, transport, HTTP stav a SHA-256 cíle/odpovědi. Rozliší nedostupnou síť, požadovanou gateway autentizaci a skutečnou Luxart dokumentaci, aniž by poslal klientské heslo.

Při použitelné dokumentaci nebo gateway výzvě probe navíc anonymně načte pouze veřejný kořen stejného originu. D1 i finální release dossier selžou, pokud tuto kontrolu nelze dokončit nebo pokud kořen stále zveřejňuje directory listing; žádný odkaz z výpisu se neotevírá.

`verify:luxart-d1` je následná jediná read-only vstupní brána integračního týdne. Pevně vyžaduje `LUXART_API_CONTRACT=memberzone_rest_v1`, přesně schválený Zone4You HTTPS origin, stejný origin pro `/Help` i API, předem schválený SHA-256 otisk originu, explicitně potvrzený gateway režim, vypnutý veřejný výpis kořenového adresáře, potvrzení IT/Luxartu, že query string `/api/Login` se v přístupových logách neukládá nebo je redigovaný, resort 1 a autentizovaný český i anglický feed včetně Reformeru. Potvrzení logů musí uvést skutečného schvalujícího člověka nebo provozní roli a čas nejvýše 30 dní před D1; placeholder ani pozdější datum neprojdou. Port není identita kontraktu: odvodí se z podepsaného originu a musí souhlasit ve všech důkazech. Současný SOAP/WCF `Service1.svc` na `api.memberzone.online:9191` proto jako REST neprojde. Skript neprovádí rezervaci či jinou mutaci a release JSON zapíše pouze do předem vytvořeného owner-only adresáře mimo repozitář bez možnosti přepsání existujícího souboru. Soubor obsahuje strojově ověřitelnou D1 atestaci verze 6 pro REST kontrakt, transport, `/Help`, vypnutý directory listing, cílový otisk, gateway režim, auditovatelnou ochranu login query logů a přihlášené čtení; release dossier schématu 6 odmítne obyčejný výstup samostatného diagnostického `verify:luxart-readonly`.

Po dodání nebo změně veřejné adresy se nejprve spustí `LUXART_HELP_URL=https://<host>:<port>/Help LUXART_EXPECTED_PORT=<port> npm run verify:luxart-anonymous-candidate`. Tento složený anonymní intake přijímá pouze přesný HTTPS cíl, ověří `/Help`, vypnutý directory listing a všech 11 částí sémantického REST kontraktu. Záměrně nečte gateway ani klientské credentials a jeho zelený výsledek pouze označí kandidáta jako způsobilý pro následnou D1 bránu; sám nikdy neautorizuje přihlášení, mutaci ani launch.

D1 odděluje i oprávnění jednotlivých kroků: první anonymní kontrola `/Help` a veřejného kořene dostane jen URL, očekávaný port a timeout, nikoli gateway nebo klientské údaje. Gateway konfigurace se načte až po potvrzení bezpečného REST/HTTPS cíle bez directory listingu, testovací klient až po úplné shodě jedenácti částí kontraktu. Chybný SOAP, HTTP nebo veřejný adresář se proto odmítne dřív, než by se citlivé údaje vůbec dostaly do příslušné části procesu.

Po D1 a doručení tabulky sálů spustí operátor `ZONE4YOU_LUXART_EVIDENCE_PATH=<owner-only D1 JSON> LUXART_RESOURCE_MAP_JSON=<serverová mapa> npm run verify:luxart-resource-map`. Mezibrána porovná mapu s přesnou množinou čísel sálů ve všech anonymních i personalizovaných CS/EN důkazech, odmítne chybějící i přidané číslo a zveřejní pouze kanonický SHA-256 mapy, nikdy interní `id_resource`. Stejnou přesnou shodu znovu vynucuje živý runtime readiness nad právě načteným feedem; při driftu vrátí `booking=resource_map_mismatch` a 503 ještě před otevřením bookingu. Úspěch není autoritou k mutaci ani cutoveru.
...
`verify:luxart-gateway-config` bezpečně ověřuje pouze způsob serverového přístupu ke konfigurovanému Luxart API. Luxart 14. 9. 2026 potvrdil stejný REST kontrakt na interním portu 9759, Zone4You testovací databázi, HTTPS, IP allowlist a Basic Auth. IT stále musí dodat přesný veřejný hostname s certifikátem a reverse proxy na `9759`. SOAP/WCF na `api.memberzone.online:9191` je jiný veřejný kontrakt a není důkazem této klientské cesty. Hodnoty Basic Auth zůstávají jen v secret store a výstup je nikdy nezobrazuje; gateway credentials nejsou totéž co testovací login klienta. Pětidenní ukládání klientského password hashe potvrzené Luxartem nesplňuje podmínku redakce, protože jde o znovupoužitelný přihlašovací údaj. Složená brána `verify:luxart-d1` proto zůstane zavřená, dokud proxy, webserver, aplikace i monitoring nebudou parametry `login`, `password` a `member_card_number` vynechávat nebo redigovat. Po bezpečném otevření D1 znovu načte český i anglický rozvrh, odmítne chybějící nebo nebinární `user_posible` a dokáže, že personalizace žádnou položku anonymního sedmidenního feedu neschovala; ukládá pouze agregované počty a hashe.
...
`verify:booking-mutations` smí běžet jen nad výslovně potvrzenou testovací databází. Ještě před prvním zápisem vyžaduje přesnou shodu staging commitu a launch fáze z `/api/readiness`, region `fra1`, live Luxart a vypnutý waitlist; u pilotu bez plateb také vypnuté top-upy. `ZONE4YOU_UAT_ROOM_SCENARIOS_JSON` musí obsahovat právě jeden různý výskyt pro každý sál v úplném `LUXART_RESOURCE_MAP_JSON` a soubor musí zahrnout běžnou lekci i Reformer. Celá sada používá jediné ověřené přihlášení, aby počet sálů nemohl narazit na login limiter; před každým sálem však znovu kontroluje readiness stejného deploye. U každého sálu skript vyžaduje explicitní personalizované povolení, autoritativní volné místo, přesnou shodu čísla sálu, potvrzené rezervační okno a bezpečné online storno s očekávaným poplatkem podle pravidla daného typu. Před zápisem i u každé odpovědi rezervace/storna používá stejné limity částek a stejnou kontrolu explicitně zónovaných časů jako živá aplikace; chybějící storno poplatek není tiše zaměněn za nulu. Výsledný UAT důkaz má povinnou verzi schématu a finální release dossier znovu nezávisle ověří úplné pokrytí mapy, oba typy lekcí, rozdílné výskyty a rezervace, částky, časy, jedinou ověřenou autentizaci a korelační ID. Odpověď na storno sama nestačí: až následný Luxart snapshot smí potvrdit, že každá UAT rezervace zmizela. Současně se u všech scénářů porovná přesná množina identit všech dříve aktivních rezervací klienta. Jakýkoli nesoulad končí bez rezervačního POSTu nebo neúspěšným UAT důkazem.
...
Checkout, podepsaný webhook i finální Luxart kreditní zápis používají společnou `paymentWriteGate`. Nestačí proto samotné Stripe secrets nebo platební ledger: před načtením Checkout požadavku a před zpracováním webhooku musí zároveň projít celý HTTPS deployment preflight, schválený region `fra1`, přesná fáze `booking_with_stripe`, gateway a query-log potvrzení, notifikační readiness, business pravidla i databázové ochrany. Červený bod zastaví požadavek před externím zápisem.
