...
`verify:luxart-d1` je následná jediná read-only vstupní brána integračního týdne. Pevně vyžaduje `LUXART_API_CONTRACT=memberzone_rest_v1`, přesně schválený Zone4You HTTPS origin, stejný origin pro `/Help` i API, předem schválený SHA-256 otisk originu, explicitně potvrzený gateway režim, potvrzení IT/Luxartu, že query string `/api/Login` se v přístupových logách neukládá nebo je redigovaný, resort 1 a autentizovaný český i anglický feed včetně Reformeru. Potvrzení logů musí uvést skutečného schvalujícího člověka nebo provozní roli a čas nejvýše 30 dní před D1; placeholder ani pozdější datum neprojdou. Port není identita kontraktu: odvodí se z podepsaného originu a musí souhlasit ve všech důkazech. Současný SOAP/WCF `Service1.svc` na `api.memberzone.online:9191` proto jako REST neprojde. Skript neprovádí rezervaci či jinou mutaci a release JSON zapíše pouze do předem vytvořeného owner-only adresáře mimo repozitář bez možnosti přepsání existujícího souboru. Soubor obsahuje strojově ověřitelnou D1 atestaci verze 5 pro REST kontrakt, transport, `/Help`, cílový otisk, gateway režim, auditovatelnou ochranu login query logů a přihlášené čtení; release dossier schématu 6 odmítne obyčejný výstup samostatného diagnostického `verify:luxart-readonly`.
