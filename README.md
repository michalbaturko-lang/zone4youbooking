...
`verify:booking-mutations` smí běžet jen nad výslovně potvrzenou testovací databází. Ještě před prvním zápisem vyžaduje přesnou shodu staging commitu a launch fáze z `/api/readiness`, region `fra1`, live Luxart a vypnutý waitlist; u pilotu bez plateb také vypnuté top-upy. Povinně provede dva oddělené scénáře nad různými výskyty: jednu běžnou lekci a jeden Reformer. U každého vyžaduje explicitní personalizované povolení, autoritativní volné místo, platné číslo sálu s release mapováním, potvrzené rezervační okno a bezpečné online storno s očekávaným poplatkem podle pravidla daného typu. Před zápisem i u každé odpovědi rezervace/storna používá stejné limity částek a stejnou kontrolu explicitně zónovaných časů jako živá aplikace; chybějící storno poplatek není tiše zaměněn za nulu. Výsledný UAT důkaz má povinnou verzi schématu a finální release dossier znovu nezávisle ověří oba typy lekcí, rozdílné výskyty, částky, sály, časy a korelační ID. Odpověď na storno sama nestačí: až následný Luxart snapshot smí potvrdit, že každá UAT rezervace zmizela. Současně se u obou scénářů porovná přesná množina identit všech dříve aktivních rezervací klienta. Jakýkoli nesoulad končí bez rezervačního POSTu nebo neúspěšným UAT důkazem.
